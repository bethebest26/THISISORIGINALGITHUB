...
const regex = /const isVol2Unlocked = \([\s\S]*?\{ \/\* Right: Personal Progression Feed \/ Achievements \*\//;

const replacement = `const isVol2Unlocked = (progress.unlockedCourses.includes(course.id) || unlockedVersions.includes("2") || unlockedVersions.includes(v2Key)) && !isV2Expired;
