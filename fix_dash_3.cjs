...
const startIndex = content.indexOf('{courses.slice(0, 4).map((course, idx) => {');
const endIndex = content.indexOf('{/* Right: Personal Progression Feed / Achievements */}');

if (startIndex !== -1 && endIndex !== -1) {
    const replacement = `{courses.slice(0, 4).map((course, idx) => {
